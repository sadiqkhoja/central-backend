...
const _get = (includeSource) => {
  const frames = [Entity, Entity.Def.into('currentVersion')];
  if (includeSource) {
    frames.push(Submission, Submission.Def.into('submissionDef'), Form);
  }
  return extender(...frames)(Actor.into('creator'), Actor.alias('current_version_actors', 'currentVersionCreator'))((fields, extend, options, deleted = false) => sql`
...
const getAll = (datasetId, options = QueryOptions.none) => ({ all }) =>
  _get(false)(all, options.withCondition({ datasetId }), isTrue(options.argData.deleted))
    .then(map((e) => {
      const defMetadata = new Entity.Def.Metadata(e.aux.currentVersion, { creator: e.aux.currentVersionCreator });
      return e.withAux('currentVersion', defMetadata);
    }));
