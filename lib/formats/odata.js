...
const extractOptions = (query) => ({
  ...
  metadata: query.$select !== '*' && query.$select?.split(',')
    ...
    .reduce((map, field) => ({ ...map, [field]: true }), {})
});
...
const serviceDocumentFor = (fields, domain, serviceUrl) => {
  ...
  for (const field of fields)
    if (field.type === 'repeat')
      tables.push(field.path.slice(1).replace(/\//g, '.'));
...
const singleRowToOData = (fields, row, domain, originalUrl, query) => {
  ...
  return submissionToOData(fields, table, row, options).then((subrows) => {
    ...
    if (options.metadata && !options.metadata.__id) {
      pared.forEach(r => delete r[filterField])
    }
  ...
  for (const property of select.split(',').map(p => p.trim())) {
    ...
    if (property.startsWith('__')) {
      if (!(property === '__id' || (table === 'Submissions' && systemFields.has(property))))
        throw Problem.user.propertyNotFound({ field: property });
    } else {
      ...
      for (let i = 0; i < propSegments.length; i+=1) {
        ...
        if (!field) throw Problem.user.propertyNotFound({ field: property });
        ...
        if (field.type === 'repeat' && i < propSegments.length - 1) throw Problem.user.unsupportedODataSelectField({ field: property });
